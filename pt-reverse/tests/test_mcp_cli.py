...
class McpCliTest(unittest.TestCase):
    ...
    def test_initialize_and_tools_list(self) -> None:
        ...
        self.assertIn("presentation", render["inputSchema"]["properties"]["preset"]["enum"])
        ...
        self.assertIn("presentation", bundle["inputSchema"]["properties"]["preset"]["enum"])
        ...
        self.assertIn("presentation", lab_plan["inputSchema"]["properties"]["preset"]["enum"])
    ...
    def test_schema_tool_returns_workflow_schemas(self) -> None:
        ...
        plan_result = responses[4]["result"]
        ...
        self.assertEqual(plan_result["isError"], False)
        self.assertIn('"kind":"pt730-plan-schema"', plan_result["structuredContent"]["stdout"])
        self.assertIn('"add-device"', plan_result["structuredContent"]["stdout"])
        self.assertNotIn("\n  ", plan_result["structuredContent"]["stdout"])
    ...
    def test_plan_tools_build_topology_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / "mcp-plan.json"
            responses = self.run_mcp(
                [
                    {
                        "jsonrpc": "2.0",
                        ...
                        "params": {
                            "name": "pt730_plan_new",
                            "arguments": {"name": "MCP Plan", "output": str(path), "compact": True},
                        },
                    },
                    {
                        "jsonrpc": "2.0",
                        ...
                        "method": "tools/call",
                        "params": {
                            "name": "pt730_plan_add_device",
                            "arguments": {"plan": str(path), "name": "R1", "category": "router", "model": "2911", "output": str(path)},
                        },
                    },
                    {
                        "jsonrpc": "2.0",
                        ...
                        "params": {
                            "name": "pt730_plan_add_device",
                            "arguments": {"plan": str(path), "name": "SW1", "category": "switch", "model": "2960-24TT", "output": str(path)},
                        },
                    },
                    {
                        "jsonrpc": "2.0",
                        ...
                        "method": "tools/call",
                        "params": {
                            "name": "pt730_plan_add_module",
                            "arguments": {"plan": str(path), "device": "R1", "slot": "0/0", "model": "HWIC-2T", "output": str(path)},
                        },
                    },
                    {
                        "jsonrpc": "2.0",
                        ...
                        "method": "tools/call",
                        "params": {
                            "name": "pt730_plan_add_link",
                            "arguments": {"plan": str(path), "a": "R1", "b": "SW1", "pa": "GigabitEthernet0/0", "pb": "FastEthernet0/1", "output": str(path)},
                        },
                    },
                    {
                        "jsonrpc": "2.0",
                        ...
                        "method": "tools/call",
                        "params": {
                            "name": "pt730_plan_add_annotation",
                            "arguments": {"plan": str(path), "target": "R1", "title": "Core", "text": "Gateway check.", "output": str(path)},
                        },
                    },
                ]
            )
            ...
            self.assertEqual(data["metadata"]["name"], "MCP Plan")
    ...
    def test_plan_wireless_tools_build_ap_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / "mcp-wireless-plan.json"
            responses = self.run_mcp(
                [
                    {
                        "jsonrpc": "2.0",
                        "id": 1,
                        "method": "tools/call",
                        "params": {"name": "pt730_plan_new", "arguments": {"name": "MCP Wireless Plan", "output": str(path)}},
                    },
                    {
                        "jsonrpc": "2.0",
                        "id": 2,
                        "method": "tools/call",
                        "params": {"name": "pt730_plan_add_device", "arguments": {"plan": str(path), "name": "AP1", "category": "accesspoint", "model": "AccessPoint-PT", "ssid": "CLASSROOM", "output": str(path)}},
                    },
                    {
                        "jsonrpc": "2.0",
                        "id": 3,
                        "method": "tools/call",
                        "params": {"name": "pt730_plan_add_ap_config", "arguments": {"plan": str(path), "name": "AP1", "ssid": "CLASSROOM", "mode": "access-point", "channel": "6", "auth": "wpa2-psk", "password": "packet123", "output": str(path)}},
                    },
                ]
            )
            for response in responses:
                self.assertEqual(response["result"]["isError"], False, response["result"]["structuredContent"].get("stderr", ""))
            data = json.loads(path.read_text(encoding="utf-8"))
            self.assertEqual(data["devices"][0]["ssid"], "CLASSROOM")
            self.assertEqual(data["ap_configs"][0]["channel"], "6")
    ...
    def test_render_tool_exposes_presentation_preset(self) -> None:
        responses = self.run_mcp(
            [
                {
                    "jsonrpc": "2.0",
        ...
        self.assertEqual(svg_result["isError"], False)
        self.assertIn("--preset", svg_result["structuredContent"]["command"])
        self.assertIn("presentation", svg_result["structuredContent"]["command"])
        self.assertIn("background: #0f172a", svg_result["structuredContent"]["stdout"])
        self.assertIn("simple-lan", svg_result["structuredContent"]["stdout"])
        self.assertNotIn("GigabitEthernet0/0", svg_result["structuredContent"]["stdout"])
        self.assertEqual(audit_result["isError"], False)
        audit = json.loads(audit_result["structuredContent"]["stdout"])
        self.assertEqual(audit["checks"]["render_options"]["preset"], "presentation")
        self.assertEqual(audit["checks"]["render_options"]["theme"], "dark")
        self.assertEqual(audit["checks"]["render_options"]["group_by"], "auto")
