...
class TemplateCliTest(unittest.TestCase):
    ...
    def test_enterprise_edge_supports_bgp_edge_routing_configs(self) -> None:
        result = self.run_template("enterprise-edge", "--name", "BGP", "--routing", "bgp")
        self.assertEqual(result.returncode, 0, result.stderr)
        plan = json.loads(result.stdout)
        joined = "\n".join(command for config in plan["ios_configs"] for command in config["commands"])
        self.assertEqual(plan["metadata"]["routing"], "bgp")
        self.assertIn("bgp_edge", plan["metadata"]["features"])
        self.assertEqual(plan["bgp_sessions"][0]["local_as"], 65001)
        self.assertEqual(plan["bgp_sessions"][0]["peer_as"], 65000)
        self.assertIn("router bgp 65001", joined)
        self.assertIn("neighbor 203.0.113.2 remote-as 65000", joined)
        self.assertIn("network 172.16.1.0 mask 255.255.255.192", joined)
        self.assertIn("network 192.168.0.0 mask 255.255.255.0", joined)
        self.assertIn("network 10.40.0.0 mask 255.255.255.0", joined)
        self.assertIn("router bgp 65000", joined)
        self.assertIn("neighbor 203.0.113.1 remote-as 65001", joined)
        self.assertIn("network 198.51.100.0 mask 255.255.255.0", joined)
        self.assertIn("ip route 10.40.0.0 255.255.255.0 10.60.0.2", joined)
        self.assertIn("ip route 0.0.0.0 0.0.0.0 10.60.0.1", joined)
        self.assertNotIn("router ospf 1", joined)
        self.assertNotIn("router eigrp 100", joined)
        self.assert_safe_and_renderable(plan)
