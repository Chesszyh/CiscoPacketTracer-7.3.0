...
pt-reverse/bin/pt730-template server-services --clients 3 --services all --domain services.local --output server-services.json
...
`server-services` uses verified `2911`, `2960-24TT`, `Server-PT`, and `PC-PT`
models. It writes a router gateway, access switch, DHCP or static clients,
Server-PT HTTP/DNS/FTP/TFTP/Email/NTP/Syslog/DHCP metadata, `server_configs`,
and service-focused verification checks. Use `--services all` or a comma list
such as `http,dns,ftp,dhcp`.
...
`add-link`/`remove-link`, and add/remove helpers for AP configs, annotations,
IPv4/IPv6 host configs, VLANs, DHCP pools, Server-PT services, IOS configs, and
security policies or the matching MCP `pt730_plan_*` tools to build and revise
custom topology JSON, serial modules, wireless metadata, and config metadata
before layout/render/safety/config export.
...
For complete lab bundles, put callouts in `render.annotations` or pass
`pt730-lab ... --annotation`; MCP `pt730_lab_template` and `pt730_lab_plan`
also accept `annotations`.
