...
- Query input schemas through MCP with `pt730_schema` before generating unfamiliar template/IP-plan/compose/config/pipeline/lab/render/IOS-template specs; use `target=render` or CLI `pt730-render schema` for render formats, options, and annotation fields.
