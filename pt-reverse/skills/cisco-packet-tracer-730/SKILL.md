...
- Build custom topology JSON incrementally with `pt730-plan new`, `add-device`, `add-module`, `add-link`, `add-ap-config`, `add-annotation`, `add-pc-config`, and the config helpers for IPv6, VLANs, DHCP pools, Server-PT services, IOS commands, and security policies when a template is not a good fit; through MCP use the matching `pt730_plan_*` tools, then run `pt730_layout`, `pt730_render`, safety, config export, or lab-bundle tools.
