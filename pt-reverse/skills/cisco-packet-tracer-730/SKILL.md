...
- Generate complete offline deliverable bundles with `pt730-lab template <lab-spec.json> --output-dir <dir>` when an agent should start from one compact template spec and write topology JSON, safety report, SVG/draw.io/HTML/Markdown/summary render bundle, per-device `.cfg` files, and a manifest; through MCP use `pt730_lab_template`.
