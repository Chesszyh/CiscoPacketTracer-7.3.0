...
`pt730_plan_set_metadata`, `pt730_plan_add_device`,
`pt730_plan_remove_device`, `pt730_plan_add_module`,
`pt730_plan_remove_module`, `pt730_plan_add_link`,
`pt730_plan_remove_link`, `pt730_plan_add_ap_config`,
`pt730_plan_add_annotation`, `pt730_plan_add_pc_config`,
`pt730_plan_add_ipv6_config`, `pt730_plan_add_vlan_config`,
`pt730_plan_add_dhcp_pool`, `pt730_plan_add_server_config`,
`pt730_plan_add_ios_config`, and `pt730_plan_add_security_policy`, allowing
agents to construct custom topology JSON, serial modules, wireless metadata,
and config metadata through reversible edits without hand-editing files before
...
topology.
The `pt730_lab_report` MCP tool writes a Markdown coursework/deliverable index
from an existing lab bundle `manifest.json`, including artifact status, safety
status, render outputs, config files, and suggested recording checks.
...
Router-on-a-stick templates generate 802.1Q router subinterfaces, switch
trunk/access ports, VLAN metadata, static or DHCP client hosts, optional router
DHCP pools, and optional per-VLAN HTTP/DNS servers.
Server-services templates generate a router gateway, access switch, Server-PT
service metadata for HTTP/DNS/FTP/TFTP/Email/NTP/Syslog/DHCP, and DHCP or
static client PCs.
...
editable.
For cleaner large diagrams, add `--theme light|dark|paper`, `--title <title>`,
