...
as `dry_run=true`.
Plan-editor MCP tools expose `pt730_plan_new`, `pt730_plan_set_metadata`,
add/remove device/module/link tools, and add/remove config metadata tools for
APs, annotations, IPv4/IPv6 host configs, VLANs, DHCP pools, Server-PT services,
IOS configs, and security policies. Agents can construct and correct custom
topology JSON through reversible edits without hand-editing files before
